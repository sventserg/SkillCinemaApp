package com.example.skillcinema.presentation.adapter.addMovieToCollection

import androidx.recyclerview.widget.RecyclerView
import com.example.skillcinema.databinding.ItemAddCollectionBinding
import com.example.skillcinema.databinding.ItemAddCollectionBottomBinding
import com.example.skillcinema.databinding.ItemAddCollectionTopBinding

class AddMovieToCollectionViewHolder(val binding: ItemAddCollectionBinding) :
    RecyclerView.ViewHolder(binding.root)

class AddMovieToCollectionTopViewHolder(val binding: ItemAddCollectionTopBinding) :
    RecyclerView.ViewHolder(binding.root)

class AddMovieToCollectionBottomViewHolder(val binding: ItemAddCollectionBottomBinding) :
    RecyclerView.ViewHolder(binding.root)
